...
export function NavUpgrade({ sx, ...other }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const storeId = workspace?.id?.toString();
        if (storeId) {
  ...
  const publicSiteHref = storeSlug
    ? paths.publicStore(storeSlug)
    : currentStoreId
    ? paths.dashboard.store.website(currentStoreId)
    : '#';
