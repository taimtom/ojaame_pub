...
import { ProductTableToolbar } from '../product-movement-table-toolbar';
import { ProductTableFiltersResult } from '../product-movement-table-filters-result';
import {
  RenderCellAdded,
  RenderCellPublish,
  RenderCellProduct,
  ...
  RenderCellPrevious,
  RenderCellCreatedAt,
} from '../product-movement-table-row';
...
// ----------------------------------------------------------------------
export function ProductMovementListView({ storeSlug: propStoreSlug, movements, loading, error, storeId, productId, emptyMessage }) {
      ...
      <DashboardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Movement' },
            { name: movements && movements.length > 0 ? movements[0].product_name : 'Unknown Product' },
          ]}
...
function applyFilter({ inputData, filters }) {
  ...
  if (publish.length) {
    inputData = inputData.filter((movement) => publish.includes(movement.status));
  }
