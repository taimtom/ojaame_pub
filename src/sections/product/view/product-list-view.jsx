...
export function ProductListView({ storeSlug: propStoreSlug }) {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.product.edit(storeSlug, id));
    },
    [router, storeSlug]
  );

  const handleAddQtyRow = useCallback(
    (id) => {
      router.push(paths.dashboard.product.addqty(storeSlug, id));
    },
    [router, storeSlug]
  );
  ...
  const columns = [
    ...
    {
      field: 'name',
      headerName: productTerm.charAt(0).toUpperCase() + productTerm.slice(1),
      flex: 1,
      minWidth: 360,
      hideable: false,
      renderCell: (params) => (
