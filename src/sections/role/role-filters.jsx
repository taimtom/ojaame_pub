...
export function RoleFilters({ open, canReset, onOpen, onClose, filters, options }) {
  ...
  const handleFilterPositions = useCallback(
    (newValue) => {
      filters.setState({ positions: newValue });
    },
    [filters]
  );
  ...
  const renderPositions = (
    <Box display="flex" flexDirection="column">
      ...
      <Autocomplete
        ...
        options={options.positions.map((option) => option)}
        getOptionLabel={(option) => option}
        value={filters.state.positions}
        onChange={(event, newValue) => handleFilterPositions(newValue)}
        renderInput={(params) => <TextField placeholder="Select Positions" {...params} />}
