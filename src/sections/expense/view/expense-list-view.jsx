...
const HIDE_COLUMNS = { expense: false };

const HIDE_COLUMNS_TOGGLABLE = ['expense', 'actions'];

// ----------------------------------------------------------------------

export function ExpenseListView({storeSlug: propStoreSlug }) {
  ...
  useEffect(() => {
    if (
      expensesError?.status === 403 ||
      expensesError?.detail === 'Not enough permissions to view expenses'
    ) {
      setStatusCode(403);
    }
  }, [expensesError]);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.expense.edit(storeSlug, id));
    },
    [router,storeSlug]
  );
...
function applyFilter({ inputData, filters }) {
  ...
  if (publish.length) {
    inputData = inputData.filter((expenses) => publish.includes(expenses.publish));
  }
