...
export function ServiceListView({ storeSlug: propStoreSlug }) {
 const { storeParam } = useParams();
  ...
  const { services, servicesLoading } = useGetServices(numericStoreId);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.service.edit(storeSlug, id));
    },
    [router, storeSlug]
  );
  ...
  const columns = [
    // { field: 'service', headerName: 'Service', filterable: false },
    {
      field: 'name',
      headerName: 'Service',
      flex: 1,
      ...
      hideable: false,
      renderCell: (params) => (
        <RenderCellService params={params}/>
      ),
    },
    {
      field: 'created_at',
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.service.new(storeSlug)}
              variant="contained"
  ...
  return (
    <>
      <GridToolbarContainer>
        <ServiceTableToolbar
          filters={filters}
          options={{ publishs: PUBLISH_OPTIONS }}
        />
...
function applyFilter({ inputData, filters }) {
  ...
  if (publish.length) {
    inputData = inputData.filter((service) => publish.includes(service.publish));
  }
