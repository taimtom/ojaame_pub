...
import { PaymentMethodTableToolbar } from '../payment-method-table-toolbar';
import { PaymentMethodTableFiltersResult } from '../payment-method-table-filters-result';
import {
  RenderCellIssuer,
  RenderCellProduct,
  RenderCellCategory,
  RenderCellCreatedAt,
} from '../payment-method-table-row';
...
const HIDE_COLUMNS = { paymentMethod: false };

const HIDE_COLUMNS_TOGGLABLE = ['payment-method', 'actions'];

// ----------------------------------------------------------------------

export function PaymentMethodListView({storeSlug: propStoreSlug }) {
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        await deletePaymentMethod(id, numericStoreId);
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      await Promise.all(selectedRowIds.map((id) => deletePaymentMethod(id, numericStoreId)));
      setTableData((prev) => prev.filter((row) => !selectedRowIds.includes(row.id)));
      toast.success('Delete success!');
    } catch (error) {
      toast.error('Failed to delete selected payment methods.');
      console.error(error);
    }
  }, [selectedRowIds, numericStoreId]);

  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.paymentMethod.edit(storeSlug, id));
    },
    [router,storeSlug]
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 140,
      editable: true,
      renderCell: (params) => <RenderCellIssuer params={params} />,
},
    {
      field: 'Description',
...
function CustomToolbar({
  ...
  return (
    <>
      ...
      {canReset && (
        <PaymentMethodTableFiltersResult
          filters={filters}
          options={{ publish: PUBLISH_OPTIONS }}
...
function applyFilter({ inputData, filters }) {
  ...
  if (publish.length) {
    inputData = inputData.filter((paymentMethods) => publish.includes(paymentMethods.is_active));
  }
