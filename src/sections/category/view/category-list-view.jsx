...
export function CategoryListView({storeSlug: propStoreSlug }) {
  ...
  const { categories, categoriesLoading, mutateCategories } = useGetCategories(numericStoreId);
  ...
  useEffect(() => {
    if (categories.length) {
      ...
      // If categories exist, assume defaults might have been added
      // We'll show the button only if categories are empty
      setDefaultsAdded(categories.length > 0);
    } else {
      setTableData([]);
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.category.edit(storeSlug, id));
    },
    [router,storeSlug]
  );

  const handleAddDefaultCategories = useCallback(async () => {
    if (!numericStoreId || addingDefaults) return;
    
    setAddingDefaults(true);
    try {
      const newCategories = await addDefaultCategories(numericStoreId);
      toast.success(`Successfully added ${newCategories.length} default categories!`);
      setDefaultsAdded(true);
      // Refresh categories by triggering a re-fetch
      if (mutateCategories) {
        await mutateCategories();
      }
    } catch (error) {
      console.error('Error adding default categories:', error);
      toast.error(error?.response?.data?.detail || error?.message || 'Failed to add default categories');
    } finally {
      setAddingDefaults(false);
    }
  }, [numericStoreId, addingDefaults, mutateCategories]);
  ...
  const CustomToolbarCallback = useCallback(
    () => (
      <CustomToolbar
        ...
        showAddDefaults={!defaultsAdded && categories.length === 0}
        addingDefaults={addingDefaults}
      />
    ),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [filters.state, selectedRowIds, defaultsAdded, categories.length, addingDefaults, handleAddDefaultCategories]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.category.new(storeSlug)}
              variant="contained"
...
function applyFilter({ inputData, filters }) {
  ...
  if (publish.length) {
    inputData = inputData.filter((categories) => publish.includes(categories.publish));
  }
