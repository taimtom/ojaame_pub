...
export const schemaHelper = {
  ...
  editor1: (props) => zod.string().optional(),
  /**
   * object
   * defaultValue === null
   */
    ...
    zod
      ...
      .transform((data, ctx) => {
        if (data === undefined || data === null || data === '') {
          // No file provided, treat as optional
          return null;
        }
  ...
  files1: (props) =>
    zod
      .array(zod.custom())
      .optional() // Mark the entire array as optional
      .transform((data, ctx) => {
        if (!data || data.length === 0) {
          // No files provided; treat it as optional without adding issues
          return [];
        }

        const minFiles = props?.minFiles ?? 2;

        if (data.length < minFiles) {
          ctx.addIssue({
            code: zod.ZodIssueCode.custom,
            message: `Must have at least ${minFiles} items!`,
          });
