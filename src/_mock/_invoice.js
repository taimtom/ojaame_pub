...
const SERVICE_ITEMS = [...Array(3)].map((__, index) => {
  ...
  return {
    ...
    type: 'service', // Add a type identifier
...
export const INVOICE_PRODUCT_OPTIONS = [...Array(8)].map((_, index) => ({
  id: _mock.id(index),
  name: _mock.productName(index),
  price: _mock.number.price(index),
}));

const PRODUCT_ITEMS = [...Array(3)].map((__, index) => {
  const total = INVOICE_PRODUCT_OPTIONS[index].price * _mock.number.nativeS(index);
  ...
  return {
    ...
    description: _mock.sentence(index),
    price: INVOICE_PRODUCT_OPTIONS[index].price,
    type: 'product', // Add a type identifier
    service: INVOICE_PRODUCT_OPTIONS[index].name,
    quantity: _mock.number.nativeS(index),
  };
});

// Merge Items Dynamically
const ALL_ITEMS = [...SERVICE_ITEMS, ...PRODUCT_ITEMS];
...
export const _invoices = [...Array(20)].map((_, index) => {
  ...
  // Filter items based on type (service or product) dynamically
  const invoiceType = index % 2 === 0 ? 'service' : 'product'; // Alternate between services and products
  const filteredItems = ALL_ITEMS.filter(item => item.type === invoiceType);
  ...
  return {
    ...
    subtotal,
    totalAmount,
    items: filteredItems, // Dynamically assign filtered items
    invoiceType, // Include the type for reference
